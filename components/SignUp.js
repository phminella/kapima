...
import { SIGNUP_MUTATION } from '../lib/queries/userQueries';
...
const SignUp = () => {
    ...
    return (
        <SignUpStyle>
            {data && <h2>You are registered! <Link href="/sign-in">Sign in!</Link></h2>}
            {!data && <><Form onSubmit={handleSubmit}>
                ...
                <fieldset disabled={loading} aria-busy={loading}>
                    ...
                    <label htmlFor="email">
                        ...
                        {emailExists() && <p className="error">&#9940;Email already exists! <Link href="/sign-in">Sign-in?</Link></p>}
                    </label>
                    {/* // Password */}
                ...
                <section id="OAuth-signup">
                    <button id="facebook-button" type="button">Sign up with Facebook</button>
                    <button id="gmail-button" type="button">Sign up with Gmail</button>
                </section></>}
        </SignUpStyle>
    )
};
